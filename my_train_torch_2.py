...
def train(layer_name, period, epochs, modelo=1):
    ...
    num_classes = 6 # my_config.NUM_CLASES_TRAIN
    ...
    ####################################################
    model = BayesianENet(modelo=3, in_features=input_shape[0], output_dim=num_classes)
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            running_corrects += (y_pred == y_batch).float().sum().item()
        ...
        with torch.no_grad():  # Desactivar el cálculo de gradientes
            for X_val_batch, y_val_batch in val_loader:
                ...
                y_val_batch = y_val_batch.argmax(axis=1)
                val_corrects += (y_val_pred == y_val_batch).float().sum().item()
